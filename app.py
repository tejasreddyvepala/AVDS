...
def find_nearest_station(drone_location, station_locations, grid_size):
    """
    ...
    grid_station_locations = [lat_lon_to_grid(station['location'][0], station['location'][1],grid_size) for station in station_locations]
...
def simulate_drone_movement(drone, station_locations,grid_size):
    """
    ...
    with open('print_outputs.txt', 'w') as f:
      ...
      nearest_station = find_nearest_station(original_location, station_locations, grid_size)
...
# Default values
    ...
    if request.method == 'POST':
        ...
        session['all_grid_cells'] = all_grid_cells
        session['num_customers'] = num_customers
        session['num_nfzs'] = num_nfzs
        session['num_stations'] = num_stations
        session['nfz_cells'] = nfz_cells
...
def simulation():
    grid_size = int(session.get('grid_size', 0))  # Default to 0 if not found
    store_cells = list(session.get('store_cells', []))
    customer_cells = list(session.get('customer_cells', []))
    station_cells = list(session.get('station_cells', []))
    lat_step = float(session.get('lat_step', 0.0))
    lon_step = float(session.get('lon_step', 0.0))
    num_stores = int(session.get('num_stores', 0))
    all_grid_cells = list(session.get('all_grid_cells', []))
    num_customers = int(session.get('num_customers', 0))
    num_nfzs = int(session.get('num_nfzs', 0))
    num_stations = int(session.get('num_stations', 0))
    nfz_cells = list(session.get('nfz_cells', []))
    ...
    def create_denton_map(store_cells, customer_cells, station_cells, nfz_cells, grid_size, lat_step, lon_step):
            ...
            return denton_map, grid

    # Call the helper function to create the map
    denton_map, grid = create_denton_map(store_cells, customer_cells, station_cells, nfz_cells, grid_size, lat_step, lon_step)
        ...
        for i in range(70):
            ...
            for drone in drones:
                if drone['battery'] < 40:
                    ...
                    simulate_drone_movement(drone, station_locations,grid_size)
    ...
    return render_template('simulation.html', denton_map = denton_map)
